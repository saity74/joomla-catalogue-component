	...
	protected function getInput()
	{
		...
		if ($allowClear && !$scriptClear)
		{
			...
			$script[] = '		document.getElementById(id + "_name").value = "' .
				htmlspecialchars(JText::_('COM_CATALOGUE_SELECT_AN_ITEM', true), ENT_COMPAT, 'UTF-8') . '";';
		...
		$html = array();
		$link = 'index.php?option=com_catalogue&amp;view=items&amp;layout=modal&amp;tmpl=component&amp;function=jSelectItem_' . $this->id;
		...
		if (empty($title))
		{
			$title = JText::_('COM_CATALOGUE_SELECT_AN_ITEM');
		}
		...
		$html[] = '<a href="#modalArticle' . $this->id . '" class="btn hasTooltip" role="button"  data-toggle="modal" title="'
			. JHtml::tooltipText('COM_CATALOGUE_CHANGE_ITEM') . '">'
		...
		if ($allowEdit)
		{
			$html[] = '<a class="btn hasTooltip' . ($value ? '' : ' hidden')
				...
				. JHtml::tooltipText('COM_CATALOGUE_EDIT_ITEM') . '" ><span class="icon-edit"></span>' . JText::_('JACTION_EDIT') . '</a>';
		}

		// Clear item button
		if ($allowClear)
		{
			$html[] = '<button id="' . $this->id . '_clear" class="btn' . ($value ? '' : ' hidden') . '" onclick="return jClearItem(\'' .
		...
		$html[] = JHtml::_(
			...
			array(
				'url' => $url,
				'title' => JText::_('COM_CATALOGUE_CHANGE_ITEM'),
				'width' => '800px',
				'height' => '300px',
