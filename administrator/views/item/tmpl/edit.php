...
<script type="text/javascript">
	...
	window.addEvent('domready', function () {
		...
		$('assocTableAdd').addEvent('click', function (e) {
			...
			var jform_item_id = $('jform_item_id'),
				assoc_id = jform_item_id.get('value'),
		...
		$('techsTableAdd').addEvent('click', function (e) {
			...
			tr.innerHTML = '<td width="1%"><input type="checkbox"/></td>' +
			'<input type="hidden" name="jform[techs][show_short][]" value="" class="pub"/>' +
			'<td class="span2"><input type="text" class="inputBox required" name="jform[techs][name][]" /></td>' +
			'<td class="span1"><input type="text" class="inputBox required" name="jform[techs][value][]" /></td>' +
			'<td class="span2"></td>';
			tr.getElement('td:last-child').adopt(remove);
			$('techsTable').adopt(tr);
		});
			...
			<table class="table table-stripped tablesorter" id="assocTable">
				...
				<?php foreach ($this->item->assoc as $assoc) : ?>
					<tr class="roww" id="row_<?php echo $assoc->assoc_id; ?>">
						<td width="1%">
							<input title=""
								   type="checkbox" <?php if ($assoc->published): ?> checked="checked" <?php endif; ?>/>
							<input type="hidden" name="jform[assoc][assoc_id][]"
								   value="<?php echo $assoc->assoc_id; ?>"/>
							<input type="hidden" name="jform[assoc][ordering][]"
								   value="<?php echo $assoc->ordering; ?>"/>
							<input type="hidden" name="jform[assoc][published][]"
								   value="<?php echo $assoc->published; ?>" class="pub"/>
						</td>
		...
		<div class="row-fluid form-horizontal-desktop">
			<div class="controls">
				<div class="span12">
					<div class="control-group">
						<div class="control-label">
							<label for="review_input_fio">ФИО</label>
						</div>
						<div class="controls">
					...
					<div class="control-group">
						<div class="control-label">
							<label for="review_input_rate">Рейтинг</label>
						</div>
						<div class="controls">
					...
					<div class="control-group">
						<div class="control-label">
							<label for="review_input_text">Текст</label>
						</div>
						<div class="controls">
			...
			<table class="table table-stripped tablesorter" id="reviewsTable">
				...
				<?php foreach ($this->item->reviews as $review) : ?>
					<tr class="review_roww" id="review_row_<?php echo $review->id; ?>">
						<td width="1%">
							<input title=""
								   type="checkbox" <?php if ($review->published): ?> checked="checked" <?php endif; ?>/>
							<input type="hidden" name="jform[reviews][ordering][]"
								   value="<?php echo $review->ordering; ?>"/>
							<input type="hidden" name="jform[reviews][published][]"
								   value="<?php echo $review->published; ?>" class="pub"/>
						</td>
						<td><input title="" type="text" name="jform[reviews][review_fio][]"
								   value="<?php echo $review->item_review_fio; ?>"/></td>
						<td><input title="" type="text" name="jform[reviews][review_rate][]"
								   value="<?php echo $review->item_review_rate; ?>"/></td>
						<td><input title="" type="text" name="jform[reviews][review_date][]"
								   value="<?php echo $review->item_review_date; ?>"/></td>
						<td><textarea title="" name="jform[reviews][review_text][]"
